...
// батарейка, активна ли свеча/монстр) создаются там же — эти данные НЕ мутируют.
const LEVELS = [
  {
    ...
    traps: [],
    // Прямая погоня в открытой части уровня.
    monsters: [
      { x: 360, y: 350 },
    ],
    // Техническая линза: доступна с обеих сторон; центральная стена ниже неё
    // позволяет проверить, что secondary light останавливается препятствием.
    lenses: [
      { x: 360, y: 250, angle: Math.PI / 2 },
    ],
  },
  {
  ...
  },
  {
    ...
    // Между стартом игрока и монстром лежит левая часть верхней стены.
    monsters: [
      { x: 400, y: 450 },
    ],
  },
